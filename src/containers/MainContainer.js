...
class MainContainer extends Component {
  ...
  displayStocks = () => {
    // creating a new array, filtering by the search term (which I modified to check fro lowercase as well)
    let displayStocks = this.state.stocks.filter((stock) =>
      stock.name.toLowerCase().includes(this.state.searchTerm)
    );
    //If the filter is not all, alter the displayStocks accordingly
    if (this.state.filter !== "All") {
      displayStocks = displayStocks.filter(
        (stock) => stock.type == this.state.filter
      );
    }
    //Then, check the sort state and return an array accordingly
